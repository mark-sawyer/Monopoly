...
public class TokenIcon : MonoBehaviour {
    ...
    public void setup(PlayerInfo player) {
        TokenSprites tokenSprites = GetPlayerScriptableObjects.tokenTypeToTokenSprites(player.Token);
        TokenColours tokenColours = GetPlayerScriptableObjects.playerColourToTokenColours(player.Colour);
        setAndAdjustImages(
